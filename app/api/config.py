...
async def save_configuration(config: WizardConfiguration) -> JSONResponse:
    ...
    try:
        ...
        env_vars = {
            ...
            "HEADSCALE_SETUP_MODE": config.headscale.setupMode.value if config.headscale.setupMode else "",
...
"""
                ...
                headplane_base_url = f"{config.headscale.serverUrl}/admin" if config.headscale.serverUrl else "http://localhost:3000/admin"
...
"""
                ...
                logger.info(
                    f"Generated Headplane config at {headplane_config_path}"
                )
            except Exception as e:
