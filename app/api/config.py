    ...
    try:
        ...
        if not os.path.exists(env_file_path):
            if os.path.exists("env.example"):
                with (
                    open("env.example") as example_file,
                ...
                logger.warning(
                    f"env.example not found. Created a blank .env file at {env_file_path}"
                )
        # Before writing .env
        ...
        try:
            ...
            if os.path.exists(slskd_template_path):
                ...
                logger.info(
                    "Generated slskd.yml from template with Soulseek credentials"
                )
            else:
                logger.warning(
        ...
        try:
            ...
            if os.path.exists(template_path):
                ...
                logger.info(
                    f"Generated {DOCKER_COMPOSE_FULL_FILE} with user music path: {host_music_path}"
                )
        ...
        def run_stack():
            try:
                success, message = runner.compose_up(
                    compose_file=DOCKER_COMPOSE_FULL_FILE, build=False, detach=True
                )
                if success:
