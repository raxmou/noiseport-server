...
export function TextInput({ label, description, error, className = '', ...props }: TextInputProps) {
  return (
    <div className="mb-4">
      {label && (
...
export function PasswordInput({ label, description, error, className = '', ...props }: PasswordInputProps) {
    ...
    <div className="mb-4">
      ...
      <div className="relative">
        <input
          ...
          className={`input w-full pr-10 ${error ? 'border-red-500' : ''} ${className}`}
          {...props}
        />
        <button
          type="button"
...
export function Checkbox({ label, className = '', ...props }: CheckboxProps) {
  return (
    <label className="flex items-center cursor-pointer group">
      <input
        type="checkbox"
        className={`w-5 h-5 rounded border-neutral-600 bg-neutral-800 text-primary focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-all ${className}`}
        {...props}
      />
      {label && (
        <span className="ml-3 text-neutral-200 group-hover:text-neutral-100 transition-colors">
          {label}
        </span>
      )}
    </label>
