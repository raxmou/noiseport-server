...
export default function SummaryStep({ config, onValidation }: Props) {
  ...
  return (
    <>
      <h2 className="text-2xl font-kode mb-4">Configuration Summary</h2>
      <p className="text-neutral-400 mb-6">
        Review your configuration before completing the setup. You can go back
        to any previous step to make changes if needed.
      </p>
      ...
      <div className="space-y-4">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <span
              className={`px-3 py-1 rounded-full text-xs font-medium ${
                enabledServices.includes("Navidrome") ||
                enabledServices.includes("Jellyfin")
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                Navidrome:{" "}
                {config.navidrome.enabled ? config.navidrome.url : "Disabled"}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">
                Jellyfin:{" "}
                {config.jellyfin.enabled ? config.jellyfin.url : "Disabled"}
              </span>
            </div>
          </div>
        </Paper>
        ...
        <Paper>
          ...
          {config.spotify.enabled ? (
            <p className="text-sm">
              Client ID: {config.spotify.clientId.substring(0, 8)}...
            </p>
          ) : (
        ...
        <Paper>
          ...
          <div className="space-y-1">
            <p className="text-sm">
              Base Path: {config.musicPaths?.hostMusicPath}
            </p>
            <p className="text-sm text-neutral-400">
              Downloads: {config.musicPaths?.hostMusicPath}/downloads
            </p>
            <p className="text-sm text-neutral-400">
              Complete: {config.musicPaths?.hostMusicPath}/complete
            </p>
          </div>
        </Paper>
        ...
        >
          <p className="text-sm">
            Once you complete the setup, these settings will be saved and the
            application will be ready to use. You can always modify these
            settings later through the application interface.
          </p>
        </Alert>
      </div>
    </>
